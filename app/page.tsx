...
export default function InfiniteScrollFeed() {
  const { posts, loading, hasMore, error, fetchPosts, loadMorePosts } = usePostStore()
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400">Scroll down to load more amazing content</p>
        </div>
        ...
        {!hasMore && posts.length > 0 && (
          <div className="mt-12 text-center">
            <div className="inline-flex items-center px-6 py-3 bg-white dark:bg-slate-800 rounded-full shadow-lg border border-slate-200 dark:border-slate-700">
              <span className="text-slate-600 dark:text-slate-400 font-medium">🎉 You've reached the end!</span>
            </div>
          </div>
        )}
